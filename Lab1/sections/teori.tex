...
\begin{equation}
    u_{tt} + \left(\frac{R}{L} + \frac{G}{C}\right)u_t + \left(\frac{RG}{LC}\right)\,u = \left(\frac{1}{\sqrt{LC}}\right) u_{xx}
\end{equation}
\begin{equation}
...
\begin{equation}
u(x,t) = \Re\!\left\{ U e^{j\omega t - \gamma(\omega)x} \right\}.
...
\[    
    u_{tt} + (\alpha + \beta)u_{t} + \alpha \beta u = c^2 u_{xx}.
\]
Ser vi at den tidslige avledningen bare gir faktorer av $j\omega$:
\[
    u_{t} = j\omega u, \quad u_{tt} = -\omega^2 u,
\]
og den romlige avledningen gir faktorer av $\gamma$:
\[
    u_{x} = -\gamma u, \quad u_{xx} = \gamma^2 u.
\]
Som gir oss:
\[    
-\omega^2 u + j\omega(\alpha+\beta)u + \alpha \beta u = c^2 \gamma^2 u.
\]
Etter innsetting og forenkling står vi igjen med sammenhengen
Mulig endre til:
Med ansatsen $u=Ue^{j\omega t -\gamma x}$ følger:
\[
